{
  ...
  system = pkgs.nixos {
    ...
    environment.etc = pkgs.lib.optionalAttrs (hardwareReport != null) {
      # An explicit mode makes the activation script copy the report instead of
      # symlinking it through /etc/static. Symlinks only resolve inside the
      # installed system, so the test could not read the report from /mnt.
      "fixture-hardware-report.json" = {
